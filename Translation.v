...
Definition Z_ASSGN (τz:gmp_t) v (z:Z) : statement := match τz with
    | C_Int => <{ v = z }>
...
Definition binop_ASSGN (fsl_op:fsl_binop_int) (v:gmp_t ⨉ 𝓥) e₁ e₂ (r:id) v₁ v₂ : statement := 
    let (τ,c) := v in
...
Record translated_statement {T: Type} := mkSTR
{
    ...
    res: @c_exp _gmp_t  ; (* variable containing the result *)
}.
...
Coercion var_default_int (v:id) := @C_Id _gmp_t v C_Int.
...
Fixpoint translate_predicate (bindings:Γᵥ) (t_inf : type_inf) (e: ψ) (p: predicate) : state  := match p with
    ...
    | Not p => 
        ...
        mkSTR statement (mkTR statement code p_tr.(tr).(env statement) nil) decl c
    ...
    | Disj p1 p2 => 
        ...
        tr_p2 <- translate_predicate bindings t_inf tr_p1.(tr).(env statement) p2 ;;;
        ...
        in
        mkSTR statement (mkTR statement code tr_p2.(tr).(env statement) nil) decl c

    | P_BinOp t1 fsl_op t2 =>
        c <- fresh_variable ;;
        tr_t1 <- translate_term bindings t_inf e t1 ;;
        let t1_code := tr_t1.(tr).(chunk statement) in
        tr_t2 <- translate_term bindings t_inf tr_t1.(tr).(env statement) t2 ;;
    ...
    | Call _ _ =>  let _ := translate_term in ret (mkSTR statement (mkTR statement Skip e nil) nil "") (* todo : same as fcall *)
end 
...
with translate_term (bindings : Γᵥ) (t_inf : type_inf) (e: ψ) (t : fsl_term) : state := match t with
    | T_Id v => let res := 
        ...
        | Some x => fst x
        | None => v 
        end  in ret (mkSTR statement (mkTR statement Skip e nil) nil v)
    ...
    | T_BinOp t1 _op t2 => 
        ...
        t2_tr <- translate_term bindings t_inf t1_tr.(tr).(env statement) t2 ;; 
    ...
    | Conditional p t1 t2 => 
        ...
        p_tr <- translate_predicate bindings t_inf e p ;;
        t1_tr <- translate_term bindings t_inf p_tr.(tr).(env statement) t1 ;; 
        let t1_code := t1_tr.(tr).(chunk statement) in
        let t1_res := t1_tr.(res) in
        t2_tr <- translate_term bindings t_inf t1_tr.(tr).(env statement) t2 ;;;
        let t2_code := t2_tr.(tr).(chunk statement) in
        ...
        mkSTR statement (mkTR statement code t2_tr.(tr).(env statement) nil) decl c
    end
