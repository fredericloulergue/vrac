...
Module Translation (Oracle : Oracle).
    ...
    Record translation_result {T : Type} := mkTR
    {
        ...
        glob:global ⃰ ; (* mini-c globals generated during translation (global var + routine) *)
    }.
    ...
    Fixpoint translate_statement (f:fenv) (bindings : Γᵥ) t_env (env: ψ) (s : fsl_statement) : @state (@translation_result gmp_statement) := 
        match s with
        | S_Ext (LAssert p)  => 
            p_tr <- translate_predicate f bindings t_env env p 100;;;
    ...
    Definition translate_program (f:fenv) (bindings : Γᵥ) t_env (p: fsl_pgrm)  : rac_pgrm := 
        ...
        let res := List.fold_left (
            fun (done : @state ((list _ * list gmp_routine) * ψ)%type) (doing:fsl_routine)  => 
                ...
                | PFun rtype name args decls body =>
                    ...
                    body_tr <- translate_statement f bindings t_env env body ;;; 
