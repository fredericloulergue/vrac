...
Module Translation (Oracle : Oracle).
    ...
    Definition ψ := (𝔏 ⨉ (𝔏 ⇀ 𝐼)) ⇀ 𝓥 . (* global logic definitions env *)
    ...
    Record translation_result {T : Type} := mkTR
    {
        ...
        glob:gmp_routine ⃰ ; (* mini-c globals generated during translation (global var + routine) *)
    }.
    ...
    Fixpoint translate_predicate (f : fenv) (bindings:Γᵥ) t_env (e: ψ) (p: predicate) (fuel:nat) : state  := 
    match fuel with 
    | O => 
    ...
    with translate_term (f : @fenv _fsl_statement Empty_set) (bindings : Γᵥ) t_env (e: ψ) (t : fsl_term) (fuel:nat) {struct fuel}
    ...
    with function_generation (f:@fenv _fsl_statement Empty_set) (bindings:  Γᵥ) t_env (env: ψ) (fname: id) (fuel:nat) {struct fuel}
    ...
    | S n =>
        ...
        | Some (params,b) => 
            ...
            *)
            let upd_binding := {{(fname,t_env)\fres}} in
    ...
    Fixpoint translate_statement (f:fenv) (bindings : Γᵥ) t_env (env: ψ) (s : fsl_statement) : @state (@translation_result gmp_statement) := 
        match s with
        | S_Ext (LAssert p)  => 
    ...
    Definition build_fenv (routines: list fsl_routine) : @fenv _fsl_statement Empty_set := 
        let extract_c_args : _c_decl ⃰ -> 𝓥 ⃰ := List.map (fun d => let 'C_Decl _ x := d in x) in
        let extract_fsl_args : fsl_decl ⃰ -> 𝓥 ⃰ := List.map (fun d => let 'FSL_Decl _ x := d in x) in
    ...
    Definition translate_program (p: fsl_pgrm) (bindings : Γᵥ) t_env : rac_pgrm := 
        ...
        let res := List.fold_left (
            fun (done : @state ((list gmp_routine * list gmp_routine) * ψ)%type) (doing:fsl_routine)  => 
                ...
                | PFun rtype name args decls body =>
                    ...
                    body_tr <- translate_statement fenv bindings t_env env body ;;; 
