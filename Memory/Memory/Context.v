...
Module Context(V : DecidableType)(B: Eqb.EQB)
  ...
  Fact wf_after_store `(Hstore: store(κ, C.(M), b, δ, v) = ⎣M2⎦) :
  ...
  Fact wf_after_malloc `(Halloc: alloc(C.(M), Z.to_nat n) = (b', M2))
    `(Hstore: store(κ, M2, b, δ, Ptr(b',0%Z)) = ⎣M3⎦) :
  ...
  Definition alloc_var (C1: context) (x: V.t) n : context :=
    let res := alloc(C1.(M), Z.to_nat n) in
    let b := fst res in
    let M2 := snd res in 
